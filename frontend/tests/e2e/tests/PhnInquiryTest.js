...
test('Check properly filled form passes validation and validate results', async t => {
  await t
    ...
    .expect(PhnInquiryPage.resultsTable.exists).ok()
    .expect(PhnInquiryPage.resultsTable.child('thead').exists).ok()
    .expect(PhnInquiryPage.resultsTable.child('tbody').child('tr').count).eql(2)
    .expect(PhnInquiryPage.resultsRow1.exists).ok() 
    .expect(PhnInquiryPage.resultsRow1.child('td').exists).ok()
    // Validate the first row
    .expect(PhnInquiryPage.resultsRow1.child('td').nth(0).textContent).eql('9873895902') 
    .expect(PhnInquiryPage.resultsRow1.child('td').nth(1).textContent).eql('GFMSRHNISNAME, GERHNIMODFNAME') 
    .expect(PhnInquiryPage.resultsRow1.child('td').nth(2).textContent).eql('19700909') 
    .expect(PhnInquiryPage.resultsRow1.child('td').nth(3).textContent).eql('M')
})
...
test('Check properly filled form passes validation and validate results (which contain a warning)', async t => {
  await t
    ...
    .expect(PhnInquiryPage.resultsTable.exists).ok()
    .expect(PhnInquiryPage.resultsTable.child('thead').exists).ok()
    .expect(PhnInquiryPage.resultsTable.child('tbody').child('tr').count).eql(1)
    .expect(PhnInquiryPage.resultsRow1.exists).ok() 
    .expect(PhnInquiryPage.resultsRow1.child('td').exists).ok()
    // Validate the first row
    .expect(PhnInquiryPage.resultsRow1.child('td').nth(0).textContent).eql('9879431603') 
    .expect(PhnInquiryPage.resultsRow1.child('td').nth(1).textContent).eql('SMITH, JENNIFER       JENNY') 
    .expect(PhnInquiryPage.resultsRow1.child('td').nth(2).textContent).eql('20000101') 
    .expect(PhnInquiryPage.resultsRow1.child('td').nth(3).textContent).eql('F') 
    .expect(PhnInquiryPage.resultsRow1.child('td').nth(4).textContent).eql('Y') 
