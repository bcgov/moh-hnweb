...
fixture(`UpdateNumberAndDept Page`)
.disablePageCaching `Test UpdateNumberAndDept`
    .beforeEach(async t => {
        await t
...
test('Check required fields validation', async t => {
    await t
        ...
        .expect(UpdateNumberAndDept.errorText.nth(0).textContent).contains(GROUP_NUMBER_REQUIRED_MESSAGE)
        .expect(UpdateNumberAndDept.errorText.nth(1).textContent).contains(PHN_REQUIRED_MESSAGE)
        .expect(UpdateNumberAndDept.errorText.nth(2).textContent).contains(GROUP_NUMBER_DEPARTMENT_REQUIRED_MESSAGE)
        .expect(UpdateNumberAndDept.errorText.nth(3).textContent).contains(GROUP_NUMBER_DEPARTMENT_REQUIRED_MESSAGE)
...
test('Check invalid phn format validation', async t => {
    await t
        ...
        .expect(UpdateNumberAndDept.errorText.nth(0).textContent).contains(INVALID_GROUP_NUMBER_ERROR_MESSAGE)
        .expect(UpdateNumberAndDept.errorText.nth(1).textContent).contains(INVALID_PHN_ERROR_MESSAGE)
...
test('Check properly filled form passes validation', async t => {
	await t
        ...
        .expect(UpdateNumberAndDept.phnInput.exists).ok();
});
...
test('Check clear button clears the form', async t => {
	await t
        ...
        .expect(UpdateNumberAndDept.phnInput.value).eql('')
        .expect(UpdateNumberAndDept.groupMemberInput.value).eql('')
        .expect(UpdateNumberAndDept.groupMemberNumberInput.value).eql('')
        .expect(UpdateNumberAndDept.departmentNumberInput.value).eql('')
});